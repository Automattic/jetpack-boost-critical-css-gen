...
/**
 * Check if there are enough valid urls against the success targets.
 *
 * @param {{min: number, max: number}} successTargets Success Targets.
 * @param {number} urlsCount Number of urls.
 * @param {number} validUrlsCount Number of valid urls.
 *
 * @return {boolean} Whether or not there are enough valid urls.
 */
function hasEnoughValidUrls( successTargets, urlsCount, validUrlsCount ) {
	const minSuccessTarget =
		urlsCount < successTargets.min
			? Math.round(
					urlsCount / ( successTargets.max / successTargets.min )
			  )
			: successTargets.min;

	return validUrlsCount >= minSuccessTarget;
}
