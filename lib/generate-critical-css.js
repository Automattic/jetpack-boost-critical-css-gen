...
const { addGetParameters } = require( './add-get-parameters' );
...
async function generateCriticalCSS( {
	...
	try {
		...
		if ( validUrls.length < successUrlsThreshold ) {
			throw new SuccessTargetError( {}, fileWarnings );
		}
		...
		for ( const url of validUrls.slice( 0, successUrlsThreshold ) ) {
			...
			// Raise an error if there was no Critical CSS in the page. It is a good sign that a problem occurred.
			if (
				Object.keys( selectorPages ).length === 0 &&
				pageSelectors.length === 0
			) {
				throw new NoCSSError( {
					url: addGetParameters(
						url,
						browserInterface.requestGetParameters
					),
				} );
			}
