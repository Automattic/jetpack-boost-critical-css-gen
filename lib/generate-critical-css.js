 ...
 * @param {number|undefined} maxUrls - stop processing if/when maxUrls is hit
 * @return {CSSFileSet} - Set of CSS files used by the urls.
 */
async function collateCssFiles( browserInterface, urls, maxUrls ) {
	...
	for ( const url of urls ) {
		try {
			...
			// Abort early if we hit the max success target early.
			successes++;
			if ( maxUrls && successes >= maxUrls ) {
				break;
			}
		} catch ( err ) {
			warnings.push( err );
...
async function generateCriticalCSS( {
	...
	const minTarget = Math.min(
		( successTargets && successTargets.min ) || 0,
		urls.length
	);
