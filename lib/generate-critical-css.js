...
async function collateCssFiles( browserInterface, urls, successUrlsThreshold ) {
	...
	for ( const url of urls ) {
		try {
			...
			if (
				'object' === typeof cssIncludes.blocks &&
				Object.keys( cssIncludes.blocks ).length
			) {
 ...
 *
 * @return {Object} - List of above the fold selectors.
 */
async function getAboveFoldSelectors( {
	...
	for ( const url of validUrls.slice( 0, successUrlsThreshold ) ) {
		...
		for ( const size of viewports ) {
			...
			pageAboveFold.selectors.forEach( ( s ) =>
				aboveFoldSelectors.add( s )
			);
			pageAboveFold.preloads.forEach( ( p ) =>
				aboveFoldPreloads.push( p )
			);
		}
	}
	...
	return { selectors: aboveFoldSelectors, preloads: aboveFoldPreloads };
}
...
async function generateCriticalCSS( {
	...
	try {
		...
		const asts = cssFiles.prunedAsts( aboveFoldSelectors.selectors );
		...
		return [ css, warnings, aboveFoldSelectors.preloads ];
	} finally {
