...
class StyleAST {
	...
	pruned( criticalSelectors ) {
		const clone = new StyleAST(
			this.css,
			csstree.clone( this.ast ),
			this.errors
		);
			...
			enter: ( declaration, item, list ) => {
				if (
					filter(
						...
						this.originalText( declaration.value )
					) === false
				) {
					list.remove( item );
				}
	 ...
	 *
	 * @param {Set<string>} asts all Abstract trees.
	 * @return {Set<string>} Set of used fonts.
	 */
	getUsedFontFamilies( asts ) {
		...
		csstree.walk( this.ast, {
			...
			enter( node ) {
				...
				// Detect font name inside CSS var() function.
				const fontName = StyleAST.getFontNameFromCSSVar( node, asts );
				if ( fontName ) {
					fontFamilies.add( fontName );
				}

				// Gather family-name values.
				const frags = csstree.lexer.findDeclarationValueFragments(
					node,
					'Type',
					'family-name'
				);
	...
	/**
	 * Extract the font name from the variable.
	 *
	 * Given a node of type Declaration, check if the declaration is for a font
	 * with a variable, and extract the font name from the variable.
	 *
	 * @param {Object} node
	 * @param {Object} asts All abstract trees, to search the variable in.
	 * @return {string} Empty string if not a font declaration.
	 */
	static getFontNameFromCSSVar( node, asts ) {
		let fontName = '';
		let whiteSpacesLeft = 0;

		if ( 'font' !== node.property && 'font-family' !== node.property ) {
			return fontName;
		}

		const propertyIsShorthand = 'font' === node.property ? true : false;

		// If it's a font css shorthand declaration, then remember how many white spaces it have.
		// Because the font-family declaration is always at the end.
		if ( propertyIsShorthand ) {
		...
		csstree.walk( node.value, {
			enter( varNode ) {
				if ( varNode.type === 'WhiteSpace' ) {
					whiteSpacesLeft--;
					return;
				}

				if ( varNode.type !== 'Function' ) {
					return;
				}

				if ( propertyIsShorthand && 0 !== whiteSpacesLeft ) {
					return;
				}

				// Check only for function var().
				if ( csstree.keyword( varNode.name ).name !== 'var' ) {
					return;
				}

				// Get all the variable names.
				const varNames = varNode.children.map( StyleAST.readValue );

				let fontsFound = [];
				// For each variable name, get the font name.
				varNames.forEach( ( varName ) =>
					fontsFound.push(
						StyleAST.getFontNameFromByVarName( varName, asts )
					)
				);

				fontsFound = fontsFound.filter( ( font ) => {
					return 'string' === typeof font && font !== '';
				} );

				// Update the first font name found.
				if ( Array.isArray( fontsFound ) && fontsFound[ 0 ] ) {
					fontName = fontsFound[ 0 ];
				}
			},
		} );
	...
	static getFontNameFromByVarName( varName, asts ) {
		let fontName = '';

		asts.forEach( ( ast ) => {
			csstree.walk( ast.ast, {
				visit: 'Declaration',
				enter( node ) {
					if ( node.property === varName ) {
						if ( node.value.type === 'Value' ) {
							fontName = csstree.generate( node.value );

							// If there are multiple fonts in the value separated by ",", then get only the first one.
							if ( fontName.includes( ',' ) ) {
								fontName = fontName.substr(
									0,
									fontName.indexOf( ',' )
								);
							}
